...
EUregioner <- as.data.frame(read.csv("https://opendata.ecdc.europa.eu/covid19/subnationalcaseweekly/csv/", na.strings = "", fileEncoding = "UTF-8-BOM",stringsAsFactors =  FALSE))
...
ggplot(bbb[which(bbb$country == "Spain" & bbb$nuts_code %in% c("ES53","ES51","ES70")),], aes(x = ugenr, y = rate_14_day_per_100k/2, color = region_name)) +
  geom_line() +
...
ggplot(bbb[which(bbb$country == "Germany" & bbb$nuts_code %notin% c("ITC1", "ITC2","ITC4", "ITF1", "ITF2", "ITF3", "ITF5", "ITF6","ITG1", "ITG2", "ITH1", "ITH2", "ITH4")),], aes(x = ugenr, y = rate_14_day_per_100k/2, color = region_name)) +
  geom_line() + # scale_color_colorblind() +
  facet_wrap(vars( region_name), ncol = 4) +
  theme(legend.position = "none") +
